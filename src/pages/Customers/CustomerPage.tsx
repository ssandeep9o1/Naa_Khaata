            ...
            {!loading && sortedAndFilteredCustomers.map((customer) => (
              <TableRow key={customer.id} hover className="customer-table-row" sx={{ cursor: 'pointer' }}>
                ...
                <TableCell onClick={() => navigate(`/customer/${customer.id}`)}>
                  <Avatar src={customer.image_url || ''} sx={{ width: 60, height: 60 }}>{customer.name[0]}</Avatar>
                </TableCell>
                <TableCell onClick={() => navigate(`/customer/${customer.id}`)} sx={{ fontWeight: 600 }}>{customer.name}</TableCell>
                <TableCell onClick={() => navigate(`/customer/${customer.id}`)} sx={{ fontWeight: 600 }}>{customer.phone}</TableCell>
                <TableCell align="right" onClick={() => navigate(`/customer/${customer.id}`)} sx={{ fontWeight: 600 }}>{customer.due_amount}</TableCell>
